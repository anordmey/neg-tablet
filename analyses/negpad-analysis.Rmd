...
model.exp1.all <- lmer(rating ~ sentence.type * condition * truth.value +
                         (1 + sentence.type * truth.value | subid) +
                         (1 + sentence.type * truth.value | item), 
                       data = d.exp1)
...
contrasts(d.exp1.trueneg$neg.syntax) <- cbind(rbind(.5, 0,-.5), rbind(0, .5, -.5))

model.exp1.trueneg <- lmer(rating ~ condition * neg.concept * neg.syntax +
                             (1 + neg.concept * neg.syntax | subid) + 
                             (1 + neg.concept * neg.syntax | item), 
                           data = d.exp1.trueneg)
...
```{r}
contrasts(d.exp1$condition) <- rbind(.5, -.5)
contrasts(d.exp1$truth) <- rbind(.5, -.5)

model.noage <- lmer(resp ~ condition * truth  
                    + (1 + truth|subid) 
                    + (1 + condition*truth|item), 
