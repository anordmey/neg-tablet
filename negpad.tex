...
As our original example demonstrates, however, negative sentences are particularly sensitive to the effects of context.  In a classic study, \citeA{wason1965} found when participants viewed an array of seven red dots and one blue dot, they were faster to complete true negative sentences such as ``Circle four is not $\rule{2cm}{0.15mm}$'' when the circle identified was the exception to a rule (i.e., the one blue dot amongst many red dots). This was only true, however, when participants had first described the array in terms of an exception to a rule, i.e., ``Circle number four is blue and the rest are red,'' suggesting that noticing the ``exceptional'' nature of a feature is a key element of this supportive context. Similarly, supportive written contexts, such as a narrative framing where the negated feature is made salient prior to reading the negative sentence, can facilitate the processing of negation \cite{glenberg1999, ludtke2006, nieuwland2008, dale2011}. In event-related potential studies, real-world contexts that draw attention to a violation of commonly held belief or expectation (i.e., ``Scuba diving isn't very dangerous'') have also been shown to reduce the N400 response (a marker of semantic unpredictability) for true negative sentences \cite{nieuwland2008}.

Although these findings suggest that context can have a powerful effect on the processing of negative sentences, it is not clear what mechanism leads to this facilitative effect. One possible mechanism is that unsupportive contexts make negative sentences difficult when they are pragmatically uninformative. In recent work (\citeNP{nordmeyer2014}; Nordmeyer \& Frank, under review), we demonstrated a graded effect of context on the processing of negation, with negative sentences processed faster as the base rate of the negated feature increased. Specifically, negative sentences that were more relevant and informative given the context were processed faster. We found that this effect of context could be reliably predicted by the probability of a speaker using the same sentence to describe the same picture. Speakers are unlikely to produce sentences that are uninformative and irrelevant, and correspondingly listeners take longer to respond to unlikely sentences.

In the current paper we examine children's and adults' felicity ratings in response to negative sentences in pragmatically supported vs. unsupported contexts, allowing us to examine whether children and adults have an explicit awareness of the relative ``goodness'' of true negative sentences in different contexts. If children and adults rate true negative sentences as more felicitous in pragmatically supported contexts, this suggests that the processing and comprehension difficulties associated with negative sentences can be explained by an awareness of general pragmatic principles. We propose that children's apparent difficulty with negation is in fact due to their sensitivity to these same pragmatic principles rather than an undeveloped semantic understanding of negation or a processing difficulty that is specific to negation. In the next section, we discuss children's difficulties with negative sentences and consider a possible pragmatic explanation for children's behavior.
...
There is some evidence that children's comprehension of negation is sensitive to contextual differences (\citeNP{devilliers1975}, see \citeNP{gualmini2004} for further discussion). In a child-friendly adaptation of \citeA{wason1965}, \citeA{devilliers1975} found that children were faster and less error-prone to complete a sentence such as ``This is not a $\rule{2cm}{0.15mm}$'' when the experimenter pointed to an object that was an exception in a set of objects (i.e., pointing to a cow amongst six horses). In the current paper, we build on these findings to examine whether these context effects are born out in children's explicit judgments of sentences, in order to further understand the mechanism by which context influences negation processing. In both \cite{wason1965} and \cite{devilliers1975}, the fact that negations about an exception to a rule were processed faster could be due to the fact that these objects are more salient, or that both adults and children have an automatic tendency to name the more common object. In this paper, we specifically examine whether the pragmatic principle of informativeness could explain this facilitating effect of context. Asking children to give explicit felicity ratings to negative sentences in informative and uninformative contexts will help us understand the extent to which children are sensitive to these pragmatic principles.
