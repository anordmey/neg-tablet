...
```{r}
all.data <- read.csv("..data/negpad_long_data.csv")

##TREAT AGE GROUP AS FACTOR!
all.data$agegroup <- factor(all.data$agegroup)
```
...
Reject subjects who didn't complete at least half of the trials
...
```{r}
#check ns 
aggregate(subid ~ agegroup, all.data, n.unique)
...
ggplot(filter(ms, sent.type=="negative"), 
       aes(x = condition, y = m)) + 
  geom_bar(stat="identity", position = "dodge") + 
  geom_linerange(aes(ymin = cil, ymax = cih), 
                 position = position_dodge(width = .9)) +
  ylab("Proportion Correct") + xlab("Condition") +
  facet_grid(agegroup ~ truth) + plot.style
...
## doesn't converge, but shows 3-way interaction
summary(glmer(correct ~ condition * truth * agegroup + 
                (1|item) +
                (1|subid), 
              family = "binomial",
              data = filter(all.data, sent.type == "negative")))

## converges, with interaction & main effects (but no main effect of age)
summary(glmer(correct ~ condition * truth + agegroup +
                (1|item) +
                (1|subid), 
              family = "binomial",
