...
Try doing this in Stan. Stan code lightly modified from (https://github.com/mclark--/Miscellaneous-R-Code/blob/master/ModelFitting/Bayesian/rstan_MixedModelBetaRegression.R)
...
data {                              
  ...
  vector[N] resp;                   // resp (was yield)
  int<lower=1,upper=L> id[N];       // subject (was batch)
  vector[N] cond;                   // condition (was temp)
}
...
parameters {
  ...
  real<lower=0> sd_beta;            // sd for cond
  vector[L] gammaIntercept;         // individual effects
  vector[L] gammaCond;              // individual effects
}
...
transformed parameters {
  ...
  for (l in 1:L){
    ...
    SlopeRE[l] <- gammaCond[l]*sd_beta ;
}
  
// model calculations
for(n in 1:N) {
  yhat[n] <- inv_logit((IntRE[id[n]] + Intercept) + (SlopeRE[id[n]] + betaCond) * cond[n]);   
  }
...
model {
  ...
  gammaCond ~ normal(0, 1);         // random slopes for each batch
  // likelihood
  resp ~ beta(A, B);
}
'
...
Model run. Options from (https://github.com/stan-dev/rstan/wiki/RStan-Getting-Started#how-to-install-rstan)
...
# ugh ugh ugh - but consecutively numbers these
model.data$subid.n <- as.numeric(as.factor(as.numeric(model.data$subid)))

dat = list(N = nrow(model.data), 
           resp=model.data$resp01, 
           cond=as.numeric(model.data$cond)-1, # make 0 = none, 1 = target
           id=model.data$subid.n,
           L=length(unique(model.data$subid))) # 10 = number of batches
```
...
test <- stan(model_code = stanmodelcode, model_name = "negpad", 
             init=0,  
             # init_r=.01,  
